...
public:
    ...
    ~TraceObject()
    {
        ...
        //        printInfo("end");
        std::string info = "elapsed: " + std::to_string(elapsed) + " ms. " + "<" + getFileName(m_strFile) + ":" +
                           std::to_string(m_Line) + " " + m_strFunc + ">";
        std::string color;
        if (elapsed >= INT_DURATION_MARK)  // 如果耗时超过了阈值,则显著标记
        {
            color = "255;0;0";
        }
...
#define TRACEOBJECT(strOpt) \
    int         elpased;          \
    TraceObject CONCAT_LINE(traceObject_, __LINE__)(strOpt, elpased, "", __FILE__, __func__, __LINE__);
#define TRACEOBJECT_PARAM1(strOpt, elpased) \
    TraceObject CONCAT_LINE(traceObject_, __LINE__)(strOpt, elpased, "", __FILE__, __func__, __LINE__);
